...
// Read a word through cache system - wrapper for perform accesses
uint64_t LoadWord(cache* acache, unsigned long long address){
  return perform_access(acache, address, 8, 0, 0, 0);
}

// Write a word through cache system - wrapper for perform accesses
void StoreWord(cache* acache, unsigned long long address,  unsigned long long value){
  perform_access(acache, address, 8, 1, value, 0);
}

// perform cache accesses (at word granularity - block accesses done as sequences)
uint64_t perform_access(cache* acache, unsigned long long address, unsigned int size, Bool write, unsigned long long value, Bool blockAccess){
  ...
  for(curset=0;curset<acache->numsets;++curset){
    ...
    for(curblock=0;curblock<acache->numways;++curblock){
      ...
      if(acache->DEBUG)
        ...
        fprintf(stdout,"{v:%u d:%u data:|",acache->sets[curset].blocks[curblock].valid,acache->sets[curset].blocks[curblock].dirty);
